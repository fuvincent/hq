...
void ScanKey(void)
{
	...
	if((GPIOGetValue(PORT1, 8) & 0x800) == 0) // "UP" key is pressed
	{
		keyfiltercnt[0]++;
	...
	if((GPIOGetValue(PORT1, 9) & 0x400) == 0) // "DOWN" key is pressed 
	{
		keyfiltercnt[1]++;
	...
	if((GPIOGetValue(PORT1, 10) & 0x100) == 0) // "LEFT" key is pressed 
	{
		keyfiltercnt[2]++;
	...
	if((GPIOGetValue(PORT1, 11) & 0x200) == 0) // "RIGHT" key is pressed 
	{
		keyfiltercnt[3]++;
